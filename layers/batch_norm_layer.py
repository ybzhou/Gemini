...
class BatchNormLayer(Layer):
    ...
    def constructLayer(self, inputShape, initParams, name, 
        ...
        x_std_expr = theano.shared(x_std_values, name='x_std')
    ...
    def fprop(self, x, mode='train'):
        if mode == 'test':
            ...
            x_sq_nc = T.sqr(x).mean(self.norm_axis, keepdims=True)
        elif mode == 'train':
            x_avg = x.mean(self.norm_axis, keepdims=True)
            x_sq_nc = T.sqr(x).mean(self.norm_axis, keepdims=True)
            ...
            running_sq_nc = theano.clone(self.params.getParameter('x_std'), share_inputs=False)
        ...
        self.x_std = x_sq_nc
        self.x_avg = x_avg
        x_std = T.sqrt(x_sq_nc - T.sqr(x_avg) + 1e-8)
        x_avg = T.addbroadcast(x_avg, *self.norm_axis)
        x_std = T.addbroadcast(x_std, *self.norm_axis)
        gamma = T.addbroadcast(self.params.getParameter('gamma'), *self.norm_axis)
